...
/* ---- ACTIVAR BRILLO AL HACER SCROLL ---- */
document.addEventListener("DOMContentLoaded", () => {
    const brand = document.getElementById('brand-shimmer');
    
    if (brand) {
        // Usamos el IntersectionObserver para saber cuándo se ve en pantalla
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Esperamos medio segundo después de que aparece para lanzar el brillo
                    setTimeout(() => {
                        brand.classList.add('animate-shine');
                    }, 500); 
                    observer.unobserve(brand); // Que solo inicie una vez
                }
            });
        }, { threshold: 0.5 }); // Se activa cuando la mitad del texto es visible
